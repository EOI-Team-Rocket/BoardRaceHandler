<template>
  <div class="create">
    <div id="leftPart">
      <!--upload de la imagen, sexo radiobutton y descripcion-->
...
export default {
    ...
    descriptionState() {
      if (this.event.description == null) {
        ...
        this.stateDescription =
          this.event.description.length > 140 ? true : false;
        return this.stateDescription;
      }
    },
    placeState() {
  ...
  methods: {
    checkForm() {
      ...
      if (!this.stateTitle && this.event.title)
        this.errors.push("El Título es demasiado corto");
      ...
      if (!this.stateDescription && this.event.description)
        this.errors.push("La Descripción es demasiado corta");
      if (!this.event.place) this.errors.push("El Lugar es requerido");
      if (!this.statePlace && this.event.place)
        this.errors.push("El Lugar es demasiado corto");
      if (!this.event.gender) this.errors.push("El Sexo es requerido");
      if (!this.event.boat_category)
        this.errors.push("La Categría de Barco es requerida");
      if (!this.event.age_category)
        this.errors.push("La Categría de Edad es requerida");
      if (!this.event.sailingClub)
        this.errors.push("El Club Naútico es requerido");
    ...
    editEvent() {
      axios
        .patch("http://localhost:3000/api/v1/events/" + this.id, this.event)
        .then(res => {
          this.$router.push("/CRUDevents");
        })
        .catch(err => {
